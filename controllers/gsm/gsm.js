...
const register = async (req, res) => {
    if(req.headers.authorization){
        try{
            ...
            if(service == "" || serial == "" || serial.length != 12 || service != "GSM Bypass"){
                ...
                db.query("INSERT INTO serials SET ?", [{serial:serial, service:service, by_user:"api.v2", date:dateFormatted}], (error, result) => {
                    if(error){
                        ...
                        .setTitle(`[GSM Bypass]: New Serial Registration`)
                        .addField('Serial', `${serial}`)
                        .addField('Service', `GSM Bypass`)
                ...
                if(err){
                    ...
                    db.query("SELECT * FROM serials WHERE serial = ?", [serial], (error, result) => {
                        if(error){
                            ...
                            if(result && result != ""){
                                if(result[0].service != "GSM Bypass"){
                                    return res.send("Your iDevice is registered but not for GSM Bypass! Contact @tedddby On Telegram. Have A Nice Day!")
                                }
                                    ...
                                    if(e){
                                        ...
                                        if(s && s != ""){
                                            if(s[0].activation_record.startsWith("Albert")){
                                                        ...
                                                        if (reqResponse.statusCode === 200) {
                                                            recDB.query("INSERT INTO records SET ?", [{
                                                                ...
                                                                if (e) {
                                                                    ...
                                                                    res.writeHead(200);
                                                                    var embed = new MessageBuilder().setTitle(`[GSM Bypass]: Successfully Activated Device`).addField('Serial', `${serial}`).setColor("#00FF00").setTimestamp();
                                                ...
                                                var embed = new MessageBuilder()
                                                .setTitle(`[GSM Bypass]: Successfully Activated Device [Cached]`)
                                                .addField('Serial', `${serial}`)
                                                .setColor("#00FF00")
                                                .setTimestamp();
                                                new Webhook("https://discord.com/api/webhooks/771184406286958603/fTHp0LI470wA09irX0Z6SmV2ucMxNNUgsrdYv2NfH6QFQN8agWxd_IErhFsTeMXEifwB").send(embed);
                                ...
                                var embed = new MessageBuilder()
                                .setTitle(`[GSM Bypass]: Unregistered Request`)
                                .addField('Serial', `${serial}`)
                                .addField('Service', `GSM Bypass`)
                                .setColor("#FF0000")
                                .setTimestamp();
...
const getRecord = (req, res) => {
    const serial = req.originalUrl.replace("/gsm/records/", "");
            ...
            if(error){
                ...
                if(result && result != ""){
                    if(result[0].service == "GSM Bypass") {
                        try{
                            recDB.query("SELECT * FROM records WHERE serial = ?", [serial], (e,s) => {
                                if(e) return res.status(502).send("Internal Server Error!");
                                    ...
                                    if(s && s != ""){
                                        var embed = new MessageBuilder()
                                        .setTitle(`Successfully fetched record for ${serial} [GSM]`)
                                        .setColor("#FFFF00")
                                        .setTimestamp();
                                        new Webhook("https://discord.com/api/webhooks/771184406286958603/fTHp0LI470wA09irX0Z6SmV2ucMxNNUgsrdYv2NfH6QFQN8agWxd_IErhFsTeMXEifwB").send(embed);
                        ...
                        return res.send("Serial Is Not Registered For GSM Bypass, Its Registered For "+result[0].service)
                    }
                }else{
